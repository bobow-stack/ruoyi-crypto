...
public class Order
    ...
    private Long contractId;
        ...
        @Excel(name = "客户端幂等ID(同用户唯一；防重复下单)")
    private String clientOrderId;

            /** 订单来源：1用户下单；2条件单触发；3系统强平；4管理员(预留) */
        @TableField("source")
        @Excel(name = "订单来源：1用户下单；2条件单触发；3系统强平；4管理员(预留)")
    private Long source;

            /** 方向：1买(BUY)；2卖(SELL) */
        @TableField("side")
        @Excel(name = "方向：1买(BUY)；2卖(SELL)")
    private Long side;

            /** 持仓方向：1BOTH(单向)；2LONG；3SHORT */
        @TableField("position_side")
        @Excel(name = "持仓方向：1BOTH(单向)；2LONG；3SHORT")
    ...
    private BigDecimal qty;
            ...
            /** 已成交数量(多数一次成交，仍保留) */
        @TableField("filled_qty")
        @Excel(name = "已成交数量(多数一次成交，仍保留)")
    private BigDecimal filledQty;

            /** 成交均价(成交后写入) */
        @TableField("avg_price")
        @Excel(name = "成交均价(成交后写入)")
    private BigDecimal avgPrice;

            /** 触发类型：1最新价；2标记价；3指数价(强平建议用标记价) */
        @TableField("trigger_type")
        @Excel(name = "触发类型：1最新价；2标记价；3指数价(强平建议用标记价)")
    ...
    private Long triggerCondition;
        ...
        @Excel(name = "触发价格(满足条件则激活/成交)")
    private BigDecimal triggerPrice;

            /** 触发所使用的价格快照ID(u_price_tick.id)，用于审计回放 */
        @TableField("trigger_tick_id")
        @Excel(name = "触发所使用的价格快照ID(u_price_tick.id)，用于审计回放")
    private Long triggerTickId;

            /** 触发激活时间(从条件单变成可执行单的时间) */
        @TableField("activated_time")
        @Excel(name = "触发激活时间(从条件单变成可执行单的时间)")
        @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
    private Date activatedTime;

            /** 冻结保证金(开仓/逐仓等；平仓一般为0) */
        @TableField("freeze_margin")
        @Excel(name = "冻结保证金(开仓/逐仓等；平仓一般为0)")
    private BigDecimal freezeMargin;

            /** 手续费币种(一般USDT) */
        @TableField("fee_asset")
        @Excel(name = "手续费币种(一般USDT)")
    private String feeAsset;

            /** 手续费率快照(成交时确定，存这里便于审计) */
        @TableField("fee_rate")
        @Excel(name = "手续费率快照(成交时确定，存这里便于审计)")
    private BigDecimal feeRate;

            /** 订单状态：0NEW；1部分成交；2全部成交；3已撤销；4已拒绝；5已过期；6已触发待执行(可选) */
        @TableField("status")
        @Excel(name = "订单状态：0NEW；1部分成交；2全部成交；3已撤销；4已拒绝；5已过期；6已触发待执行(可选)")
    private Long status;

            /** 拒单原因(状态=REJECTED时) */
        @TableField("reject_reason")
        @Excel(name = "拒单原因(状态=REJECTED时)")
    private String rejectReason;

            /** 关联条件单ID(u_conditional_order.id)，来源=2时常用 */
        @TableField("related_conditional_id")
        @Excel(name = "关联条件单ID(u_conditional_order.id)，来源=2时常用")
    private Long relatedConditionalId;

            /** 关联持仓ID(u_position.id)(止盈止损/强平平仓时常用) */
        @TableField("related_position_id")
        @Excel(name = "关联持仓ID(u_position.id)(止盈止损/强平平仓时常用)")
